...
export const createOrder = async (req: Request, res: Response) => {
  try {
    ...
    if (!items || !Array.isArray(items) || items.length === 0) {
      throw new AppError(
        400,
        "At least one gear item is required to place an order",
      );
    }
    ...
    for (const item of items) {
      ...
      totalPrice += Number(gear.pricePerDay) * qty * rentalDays;
...
// 4. For Provider (getProviderOrders - GET /api/provider/orders)
...
export const updateOrder = async (req: Request, res: Response) => {
  try {
    ...
    if (order.status !== OrderStatus.PLACED) {
      throw new AppError(
        400,
        "Cannot update an order that is not in PLACED state",
      );
    }
    ...
    if (startDate && endDate) {
      ...
      for (const item of order.orderItems) {
        newTotalPrice +=
          Number(item.gear.pricePerDay) * item.quantity * rentalDays;
      }
